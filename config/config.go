...
func Load() Config {
	...
	cfg.PostgresUser = cast.ToString(getValueOrDefault("POSTGRES_USER", "ilyosbektemirov"))
	cfg.PostgresDatabase = cast.ToString(getValueOrDefault("POSTGRES_DATABASE", "market_system"))
	cfg.PostgresPassword = cast.ToString(getValueOrDefault("POSTGRES_PASSWORD", "12345"))
